...
const AppContent: React.FC = () => {
  ...
  const [activePath, setActivePath] = useState('home'); // Logic Change: Default to 'home' to keep Landing Page visible
      ...
      if (firebaseUser) {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            // Logic Change: Removed automatic setActivePath
            // We now wait for user interaction or respect 'home' state
        ...
        setCurrentUser(null);
        setActivePath('home'); // Reset to home on logout
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleEnterWorkspace = () => {
    ...
    // Route based on role MANUALLY when user clicks 'Enter Workspace'
    if (currentUser.role === UserRole.EMPLOYEE) setActivePath('wellness');
    else if (currentUser.role === UserRole.MANAGER) setActivePath('team');
    else if (currentUser.role === UserRole.HR) setActivePath('recruitment');
  ...
  // Logic Change: Keep Landing Page if user is logged in BUT activePath is 'home'
  if (!currentUser || activePath === 'home') {
    return <LandingPage onLogin={handleEnterWorkspace} initialView={landingPageMode} />;
  }
